...
def main(args):
    ...
    def fit_transform(X):
        ...
        Xs = np.zeros_like(X); Xs[idx_tr], Xs[idx_va], Xs[idx_te] = Xt, Xv, Xe
        return Xs
    ...
    for Xs_np in [X1s, X2s, X3s]:
        ...
        es.append(edge_index); ews.append(edge_attr)
    ...
    mask_va = torch.zeros_like(mask_tr); mask_te = torch.zeros_like(mask_tr)
    ...
    print(f"BEST thr={best_thr_final:.2f} | ACC={best_acc:.3f} | F1={f1:.3f} | PREC={prec:.3f} | "
          f"REC={rec:.3f} | SPEC={spec:.3f} | AUROC={auc:.3f}")
